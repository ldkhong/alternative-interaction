  ...
  */
  ...
  ngOnInit(): void{
    handTrack.load(this.modelParams).then((lmodel: any) => {
        this.model = lmodel;
        console.log("loaded");
       // let element:HTMLElement = document.getElementById('startBtn') as HTMLElement;
        //element.click();
    });
  }
  ...
  stopDetection(){
    ...
    this.onPrediction.emit(new PredictionEvent("None"))
  }
  ...
  runDetection(){
    if (this.model != null){
        let predictions = this.model.detect(this.video.nativeElement).then((predictions: any) => {
            ...
            //customize gestures! Open Hand and Closed Hand, Hand Pointing and Closed Hand
            if(openhands == 1 && closedhands == 1)
              this.detectedGesture = "Open Hand and Closed Hand";

            else if(pinching == 1 && closedhands == 1)
              this.detectedGesture = "Hand Pinching and Closed Hand";
